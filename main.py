    ...
    for i in tqdm(range(n_rep)):
        ...
        for model in models:
            ...
            res_dnc_lda = divide_n_conquer_lda(model, X_train_scaled.copy(), X_test_scaled.copy(), y_train.copy(),
                                               y_test.copy(), smote=smote_inst_1)
            # res_dnc_rda = divide_n_conquer_lda(model, X_train_scaled.copy(), X_test_scaled.copy(), y_train.copy(),
                                               # y_test.copy(), discriminate_analysis="RDA", smote=smote_inst_1)
            res_dnc_kda = divide_n_conquer_lda(model, X_train_scaled.copy(), X_test_scaled.copy(), y_train.copy(),
                                               y_test.copy(), discriminate_analysis="KDA", smote=smote_inst_1)
            res_ovo_lda = ovo_lda(model, X_train_scaled.copy(), X_test_scaled.copy(), y_train.copy(),
                                               y_test.copy(), smote=smote_inst_1)
            # res_ovo_rda = ovo_lda(model, X_train_scaled.copy(), X_test_scaled.copy(), y_train.copy(),
            #                       y_test.copy(), discriminate_analysis="RDA", smote=smote_inst_1)
            res_ovo_kda = ovo_lda(model, X_train_scaled.copy(), X_test_scaled.copy(), y_train.copy(),
                                  y_test.copy(), discriminate_analysis="KDA", smote=smote_inst_1)
            ...
            res_df = pd.concat([res_df, res_bin, res_dnc_lda, res_dnc_kda, res_ovo_lda, res_ovo_kda, res_ovo,
...
if __name__ == "__main__":
    ...
    # SMOTE_INST_1 = BorderlineSMOTE()
    SMOTE_INST_1 = BorderlineSMOTE(kind='borderline-1', sampling_strategy={1: 100, 2: 100, 3: 100, 4: 100},
                                   k_neighbors=1)
    SMOTE_INST_2 = ADASYN()
    SMOTE_INST_3 = SMOTE()
